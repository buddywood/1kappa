...
import { createPromoter, createProduct, createSeller, getAllChapters, createEvent } from './queries';
...
async function seedOrders(): Promise<void> {
  ...
  try {
    // Get some products and chapters to create orders
    const productsResult = await pool.query(
      'SELECT id, price_cents, sponsored_chapter_id FROM products LIMIT 10'
    );
    ...
    for (let i = 0; i < Math.min(orderAmounts.length, products.length); i++) {
      ...
      // Use product's sponsored_chapter_id if available, otherwise use random chapter
      const chapterId = product.sponsored_chapter_id || chapter.id;
