...
async function seedCollegiateChapters(queryInterface) {
  ...
  if (collegiateCount > 0) {
    console.log(`📖 Collegiate chapters already exist (${collegiateCount} found), skipping web scrape\n`);
    return;
  }
  ...
  try {
    ...
    let scriptPath = path.resolve(__dirname, '../../scripts/populate-collegiate-chapters.ts');
    if (!fs.existsSync(scriptPath)) {
      // Fallback: resolve from process.cwd()
      scriptPath = path.resolve(process.cwd(), 'src/scripts/populate-collegiate-chapters.ts');
    }
    if (!fs.existsSync(scriptPath)) {
      // Another fallback: resolve from backend directory
      scriptPath = path.resolve(process.cwd(), 'backend/src/scripts/populate-collegiate-chapters.ts');
    }
    ...
    console.error('  ⚠️  Warning: Failed to scrape collegiate chapters:', error.message);
    console.log('  💡 You can run "npm run populate:collegiate-chapters" manually later\n');
  }
}
...
async function updateProvincesForChapters(queryInterface) {
  ...
  console.log(`  Found ${chapters.length} chapters with null province but have state`);
  ...
  if (notFoundStates.size > 0) {
    console.log(`  States without province mapping: ${Array.from(notFoundStates).join(', ')}`);
  }
}
  ...
  async up(queryInterface, Sequelize) {
    ...
    for (const chapterData of alumniChapters) {
      try {
        ...
        console.error(`  Error inserting chapter ${chapterData.name}:`, error.message);
        errors++;
      }
    }

    console.log(`  ✅ Alumni chapters: ${inserted} inserted, ${skipped} skipped, ${errors} errors\n`);
