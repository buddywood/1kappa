...
// Use production URL for email templates (logo needs to be publicly accessible)
const FRONTEND_URL =
...
async function updateCognitoEmailTemplates() {
  try {
    ...
    if (!COGNITO_USER_POOL_ID) {
      console.error(
        "❌ COGNITO_USER_POOL_ID is not set in environment variables"
      );
    ...
    console.error(
      "\n❌ Error updating Cognito email templates:",
      error.message
    );
