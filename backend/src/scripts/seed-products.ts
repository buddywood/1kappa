...
async function seedProducts(): Promise<void> {
  ...
  try {
    ...
    if (collegiateChapters.length === 0) {
      console.error(
        "No active collegiate chapters found. Please seed chapters first."
      );
      return;
    }
    ...
    for (const sellerData of sampleSellers) {
      ...
      if (existingSeller.rows.length > 0) {
        console.log(
          `Seller ${sellerData.email} already exists, using existing seller`
        );
        ...
        const randomChapter =
          collegiateChapters[
            Math.floor(Math.random() * collegiateChapters.length)
          ];
