  ...
  try {
    ...
    if (memberResult.rows.length > 0) {
      ...
      const stewardResult = await pool.query(
        "SELECT id, fraternity_member_id, user_id FROM stewards WHERE fraternity_member_id = $1",
        [memberId]
      );
    ...
    if (!steward) {
      console.log(
        "❌ No steward found with that email or fraternity_member_id"
      );
      return;
    }

    console.log(
      `📋 Found steward (ID: ${steward.id}, fraternity_member_id: ${steward.fraternity_member_id})`
    );

    // Get fraternity_member info for display
    const memberInfo = await pool.query(
      "SELECT name, email FROM fraternity_members WHERE id = $1",
      [steward.fraternity_member_id]
    );
    ...
    const listingsResult = await pool.query(
      "SELECT COUNT(*) as count FROM steward_listings WHERE steward_id = $1",
      [steward.id]
    );
    ...
    if (listingCount > 0) {
      console.log(
        `⚠️  Warning: This steward has ${listingCount} listing(s). They will be deleted as well.`
      );
      ...
      if (claimCount > 0) {
        console.log(
          `⚠️  Warning: This steward has ${claimCount} claim(s) on their listings. They will be deleted as well.`
        );
      }
    }
    ...
    if (steward.user_id) {
      const userResult = await pool.query(
        "SELECT id, email, role, fraternity_member_id FROM users WHERE id = $1",
        [steward.user_id]
      );
      if (userResult.rows.length > 0) {
        user = userResult.rows[0];
        console.log(
          `⚠️  Warning: This steward is linked to user account: ${user.email}`
        );
    ...
    if (user) {
      if (user.role === "STEWARD") {
        ...
        console.log(`✅ Changed user role to GUEST`);
      } else {
        // User has a different role, no need to update since steward_id doesn't exist in users table anymore
        console.log(`✅ User account will remain with role: ${user.role}`);
      }
    }
    ...
    console.log(
      `✅ Deleted steward (fraternity_member_id: ${steward.fraternity_member_id})`
    );
  } catch (error: any) {
