...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const existingSteward = await getStewardByFraternityMemberId(
        fraternityMemberId
      );
      if (existingSteward) {
        return res
          .status(400)
          .json({ error: "You have already applied to be a steward" });
      }
      ...
      if (member.verification_status === "VERIFIED") {
        ...
        if (
          ...
          console.warn(
            "⚠️  Stripe not configured - auto-approving steward without Stripe account. Stripe setup will be required later."
          );
          ...
          console.log(
            `Auto-approved verified member steward (without Stripe): ${
              member.name || "Unknown"
          ...
          const warning =
            "Your steward application was approved, but Stripe payment setup is required before you can receive payments. An admin will help you complete Stripe setup.";
        ...
        try {
          ...
          console.log(
            `Auto-approved verified member steward: ${
              member.name || "Unknown"
            } (fraternity_member_id: ${member.id})`
          );
          ...
          let warningMessage =
            "Your steward application was approved, but Stripe payment setup failed. An admin will help you complete Stripe setup before you can receive payments.";

          if (
            error.type === "StripePermissionError" ||
            error.code === "secret_key_required" ||
            error.message?.includes("publishable API key")
          ) {
            console.error(
              "❌ CRITICAL: STRIPE_SECRET_KEY is set to a publishable key instead of a secret key!"
            );
            console.error(
              '   Secret keys start with "sk_" (e.g., sk_test_...)'
            );
            console.error(
              '   Publishable keys start with "pk_" (e.g., pk_test_...)'
            );
            console.error(
              "   Please update your .env file with the correct STRIPE_SECRET_KEY."
            );
            warningMessage =
              "Your steward application was approved, but Stripe payment setup failed because the server is configured with a publishable key instead of a secret key. Please contact an administrator to fix the Stripe configuration.";
          } else if (
            error.type === "StripeAuthenticationError" ||
            error.message?.includes("Invalid API Key")
          ) {
            warningMessage =
              "Your steward application was approved, but Stripe payment setup failed due to an invalid API key. An admin will help you complete Stripe setup before you can receive payments.";
          }
          ...
          console.warn(
            "⚠️  Stripe setup failed - approving steward without Stripe account"
          );
          await pool.query(
            "UPDATE stewards SET status = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2",
            ["APPROVED", steward.id]
  ...
  async (req: MulterRequest, res: Response) => {
    try {
      ...
      if (!steward || steward.status !== "APPROVED") {
        return res
          .status(403)
          .json({ error: "Steward must be approved to create listings" });
      }
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const totalListings = parseInt(
        listingsResult.rows[0]?.total_listings || "0"
      );
      ...
      const activeListings = parseInt(
        activeListingsResult.rows[0]?.active_listings || "0"
      );
      ...
      const totalDonationsCents = parseInt(
        donationsResult.rows[0]?.total_donations_cents || "0"
      );
  ...
  async (req: MulterRequest, res: Response) => {
    try {
      ...
      if (req.body.shipping_cost_cents !== undefined)
        updates.shipping_cost_cents = parseInt(req.body.shipping_cost_cents);
      if (req.body.chapter_donation_cents !== undefined)
        updates.chapter_donation_cents = parseInt(
          req.body.chapter_donation_cents
        );
...
router.get("/marketplace/public", async (req: Request, res: Response) => {
  try {
    ...
    console.log(
      `[Steward Marketplace] Found ${listings.length} active listings`
    );
    ...
    const enrichedListings = await Promise.all(
      listings.map(async (listing) => {
        try {
          ...
          let chapter = null;
          if (listing.sponsoring_chapter_id) {
            const chapterResult = await pool.query(
              "SELECT * FROM chapters WHERE id = $1",
              [listing.sponsoring_chapter_id]
            );
          ...
          console.error(`Error enriching listing ${listing.id}:`, listingError);
          // Return listing without enrichment rather than failing completely
          return {
            ...listing,
            steward: null,
            chapter: null,
    ...
    res.status(500).json({
      error: "Failed to fetch steward marketplace",
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (listing.status !== "ACTIVE") {
        return res
          .status(400)
          .json({ error: "Listing is not available for claiming" });
      }

      // Mark listing as claimed (actual payment happens in checkout)
      const claimedListing = await claimStewardListing(
        listingId,
        fraternityMemberId
      );
