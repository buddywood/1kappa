...
import {
  ...
  createUser,
  linkUserToMember,
  updateUserOnboardingStatusByCognitoSub,
  upsertUserOnLogin,
  updateUserRoleByCognitoSub,
...
router.post("/cognito/signup", async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      console.log(
        "Could not check for existing user, proceeding with signup:",
        checkError.message
      );
    }
  ...
  try {
    ...
    try {
      ...
      if (userStatus === "CONFIRMED") {
        console.log(
          `✅ User ${email} is already confirmed, skipping confirmation`
        );
        ...
        if (cognito_sub) {
          try {
            ...
            if (!user) {
              ...
              await updateUserOnboardingStatusByCognitoSub(
                cognito_sub,
                "COGNITO_CONFIRMED"
              );
            }
      ...
      if (statusError.name !== "UserNotFoundException") {
        console.error(
          "Error checking user status before confirmation:",
          statusError
        );
      }
    ...
    try {
      ...
      if (
        confirmError.name === "NotAuthorizedException" &&
        confirmError.message?.includes("already confirmed")
      ) {
        console.log(
          `ℹ️ User ${email} is already confirmed (from error message)`
        );
        confirmationSucceeded = true;
        ...
        console.error(
          `⚠️ ConfirmSignUpCommand failed for ${email}:`,
          confirmError.name,
    ...
    if (confirmationSucceeded) {
      try {
        ...
        userStatus = userDetails.UserStatus || "UNKNOWN";
        console.log(
          `📋 User status after confirmation: ${userStatus} for ${email}`
        );

        if (userStatus !== "CONFIRMED") {
          console.warn(
            `⚠️ Warning: User ${email} status is ${userStatus}, not CONFIRMED after verification`
          );
          // Use AdminConfirmSignUpCommand as a fallback to force confirmation
          console.log(
            `🔄 Attempting to force confirm user ${email} using AdminConfirmSignUpCommand`
          );
          try {
            ...
            console.error(
              `❌ Failed to force-confirm user ${email}:`,
              adminConfirmError.message
            );
            return res.status(500).json({
              error:
                "Account verification completed but status check failed. Please try logging in or contact support.",
        ...
        console.error(
          "Error checking user status after confirmation:",
          statusError
        );
    ...
    // Note: Role will be set later via the set-role endpoint
    if (cognito_sub) {
      try {
        let user = await getUserByCognitoSub(cognito_sub);
        if (!user) {
          // Create user record with COGNITO_CONFIRMED status and default GUEST role
          // Role will be updated when user selects their role
          await createUser({
            cognito_sub: cognito_sub,
            email: email,
            role: "GUEST",
...
// Set user role after PIN confirmation
router.post("/cognito/set-role", async (req: Request, res: Response) => {
  try {
    const { cognito_sub, email, role } = req.body;

    if (!cognito_sub || !email || !role) {
      return res
        .status(400)
        .json({ error: "cognito_sub, email, and role are required" });
    }

    // Validate role
    const validRoles = ["GUEST", "MEMBER", "SELLER", "PROMOTER", "STEWARD"];
    if (!validRoles.includes(role)) {
      return res
        .status(400)
        .json({
          error:
            "Invalid role. Must be one of: GUEST, MEMBER, SELLER, PROMOTER, STEWARD",
        });
    }
    ...
    try {
      let user = await getUserByCognitoSub(cognito_sub);
      if (!user) {
        // Create user if it doesn't exist (shouldn't happen, but handle it)
        ...
        // Update user role
        await updateUserRoleByCognitoSub(
          cognito_sub,
          role as
            | "ADMIN"
            | "SELLER"
...
router.post("/cognito/signin", async (req: Request, res: Response) => {
  try {
    ...
    if (!IdToken || !AccessToken) {
      return res
        .status(401)
        .json({ error: "Failed to get authentication tokens" });
    }
    ...
    const { getFraternityMemberId } = await import(
  ...
  try {
    ...
    if (
      ...
      try {
        ...
        return res.status(400).json({
          error:
            "Your email address has not been verified. Please verify your email first or request a new verification code.",
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const parsedData: any = {
        ...req.body,
        initiated_chapter_id: req.body.initiated_chapter_id
          ? parseInt(req.body.initiated_chapter_id)
          : null,
        initiated_season: req.body.initiated_season || null,
        initiated_year: req.body.initiated_year
          ? parseInt(req.body.initiated_year)
          : null,
        // Convert string 'true'/'false' to boolean, or keep boolean as-is, or undefined if not provided
        address_is_private:
          req.body.address_is_private !== undefined
            ? req.body.address_is_private === "true" ||
              req.body.address_is_private === true
            : undefined,
        phone_is_private:
          req.body.phone_is_private !== undefined
            ? req.body.phone_is_private === "true" ||
              req.body.phone_is_private === true
            : undefined,
      ...
      if (existingDraft.rows.length > 0) {
        ...
        const draftRecords = existingDraft.rows.filter(
          (row: any) => row.registration_status === "DRAFT"
        );
        const existingMember =
          draftRecords.length > 0
            ? draftRecords.find(
                (row: any) => row.cognito_sub === cognito_sub
              ) ||
              draftRecords.find((row: any) => row.email === email) ||
              draftRecords[0]
        ...
        const result = await pool.query(
          `UPDATE fraternity_members SET ${updateFields.join(
            ", "
        ...
        if (!email) {
          return res
            .status(400)
            .json({ error: "Email is required for new draft" });
        }
      ...
      if (error.code === "23505") {
        ...
        userMessage =
          "This email is already registered. Please use a different email or contact support if you believe this is an error.";
        statusCode = 409;
      } else if (error.code === "23503") {
        // Foreign key constraint violation
        userMessage =
          "Invalid information provided. Please check your chapter selection and try again.";
        statusCode = 400;
        ...
        userMessage =
          "There was an issue processing your information. Please refresh the page and try again.";
        statusCode = 400;
      } else if (error.message?.includes("duplicate key")) {
        userMessage =
          "This information is already in use. Please check your details and try again.";
        statusCode = 409;
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const parsedBody = {
        ...
        initiated_year:
          req.body.initiated_year && req.body.initiated_year !== ""
            ? parseInt(req.body.initiated_year)
            : null,
        ...
        profession_id:
          req.body.profession_id && req.body.profession_id !== ""
      ...
      if (body.cognito_sub) {
        ...
        if (existingDraft.rows.length > 0) {
          existingHeadshotUrl = (
            existingDraft.rows[0] as { headshot_url: string | null }
          ).headshot_url;
        }
      }
      ...
      if (sellerCheck.rows.length > 0) {
        ...
        if (memberCheck.rows.length > 0) {
          return res.status(400).json({
            error:
              "You already have a member profile linked to your seller account",
          });
        }
      ...
      if (existingSeller) {
        console.log(
          `Seller ${existingSeller.id} is linked to member ${member.id} via email matching (${body.email})`
        );
      }
        ...
        try {
          ...
          if (!user) {
            ...
            if (existingSeller && userRole === "SELLER") {
              const { linkUserToSeller } = await import(
          ...
          try {
            ...
            console.log(
              `Cleaned up orphaned member record ${member.id} due to user linking failure`
            );
          } catch (cleanupError) {
            console.error(
              "Failed to clean up orphaned member record:",
              cleanupError
            );
          }
      ...
      if (error.code === "23505") {
        ...
        if (error.constraint?.includes("email")) {
          ...
          userMessage =
            "This membership number is already in use. Please verify your membership number or contact support.";
        } else {
          userMessage =
        ...
        // Foreign key constraint violation
        if (
          error.constraint?.includes("chapter") ||
          error.detail?.includes("chapter")
        ) {
          userMessage =
            "The selected chapter is invalid. Please select a valid chapter and try again.";
        } else if (
          error.constraint?.includes("profession") ||
          error.detail?.includes("profession")
        ) {
          userMessage =
            "The selected profession is invalid. Please select a valid profession and try again.";
        } else {
          userMessage =
            "Some of the information provided is invalid. Please review your entries and try again.";
        }
        ...
        userMessage =
          "There was an issue uploading your headshot image. Please try uploading a different image or continue without a headshot.";
        statusCode = 500;
      } else if (
