  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!req.user.promoterId) {
        return res
          .status(403)
          .json({ error: "You must be an approved promoter to create events" });
      }
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      // Get total potential revenue (sum of ticket prices for upcoming events)
      // Note: Without max_attendees, we can only show ticket prices, not potential revenue
      const revenueResult = await pool.query(
        `SELECT COALESCE(SUM(ticket_price_cents), 0) as total_ticket_price_cents
       FROM events
       WHERE promoter_id = $1 AND event_date >= CURRENT_DATE`,
        [req.user.promoterId]
      );
      const potentialRevenueCents = parseInt(
        revenueResult.rows[0]?.total_ticket_price_cents || "0"
      );
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (event.promoter_id !== req.user.promoterId) {
        return res
          .status(403)
          .json({ error: "You can only promote your own events" });
      }
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to create promotion payment" });
    }
  }
);
