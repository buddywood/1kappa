  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        if (eventData.promoter_id !== promoterData.id) {
        ...
        setSelectedChapterId(eventData.sponsored_chapter_id);
        ...
        setFormData({
          ...
          recurrence_frequency: (() => {
            if (!eventData.recurrence_rule) return "weekly";
            const ruleStr = eventData.recurrence_rule;
          ...
          recurrence_end_date: eventData.recurrence_end_date 
            ? new Date(eventData.recurrence_end_date).toISOString().split("T")[0]
            : "",
        });
    ...
    loadData();
  }, [isAuthenticated, token, user, eventId]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (formData.is_recurring) {
        ...
        formDataToSend.append("recurrence_rule", "");
        formDataToSend.append("recurrence_end_date", "");
      }
      ...
      const result = await updateEvent(token, eventId, formDataToSend);
  ...
  if (loading) {
    return (
      <View style={styles.container}>
        <ScreenHeader title="Edit Event" onBack={onBack} />
    ...
    <View style={styles.container}>
      ...
      >
          ...
          {imageUri ? (
            ...
            <View style={styles.imagePreviewContainer}>
              ...
              <Text style={[styles.helperText, { marginTop: 8 }]}>
                Tap below to replace this image
              </Text>
            </View>
          ) : null}
          {!imageUri && (
            ...
            >
              ...
              <Text style={styles.imageUploadText}>
                {existingImageUrl ? "Tap to replace image" : "Tap to add image"}
              </Text>
            </TouchableOpacity>
          )}
        </View>
        ...
        <View style={styles.buttonContainer}>
          <PrimaryButton
            title={submitting ? "Updating Event..." : "Update Event"}
