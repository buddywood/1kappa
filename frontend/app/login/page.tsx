...
function LoginPageContent() {
  ...
  useEffect(() => {
    ...
    if (errorParam) {
      ...
      if (
        errorParam === "UserNotConfirmedException" ||
        errorParam.includes("UserNotConfirmedException")
      ) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!checkResult.success) {
        ...
        const isUserNotConfirmed =
          ...
          (errorString.includes("UserNotConfirmedException") &&
            !errorString.includes("UserNotConfirmedException is not"));

        if (isPasswordChangeRequired) {
          console.log(
            "✅ Password change required detected, showing form and preventing NextAuth call"
          );
        ...
        if (isUserNotConfirmed) {
          console.log(
            "✅ UserNotConfirmedException detected in Cognito check, showing verification form"
          );
        ...
        console.log(
          "⚠️ Not a password change or verification error, will try NextAuth. Error:",
          errorString
        );
      ...
      if (!passwordChangeRequired) {
        ...
        if (result?.error) {
          console.log(
            "NextAuth error result FULL:",
            JSON.stringify(result, null, 2)
          ); // Debug log - check full result object
          ...
          if (
            errorString.includes("NEW_PASSWORD_REQUIRED") ||
            errorString === "NEW_PASSWORD_REQUIRED"
          ) {
            setNeedsPasswordChange(true);
            setLoading(false);
            return;
          ...
          if (isUserNotConfirmed) {
            console.log(
              "✅ User not confirmed detected from NextAuth, showing verification form"
            );
            setNeedsVerification(true);
            setLoading(false);
            return;
          ...
          console.log(
            "Generic error, showing error message. Error was:",
            errorString
          );
      ...
      if (isUserNotConfirmed) {
        console.log(
          "✅ UserNotConfirmedException detected in catch block, showing verification form"
        );
        setNeedsVerification(true);
        setLoading(false);
        return;
  ...
  const handleResendVerification = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/api/members/cognito/resend-verification`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          ...
          <>
            ...
            <div className="mt-6 mb-4">
              <div className="relative">
                ...
                <div className="relative flex justify-center text-sm">
                  <span className="px-4 bg-white text-midnight-navy/60 font-medium">
                    OR LOGIN WITH
                  </span>
                </div>
              </div>
            ...
            <div className="grid grid-cols-4 gap-3 mb-10">
              ...
              >
                ...
                <span className="absolute -bottom-6 left-1/2 -translate-x-1/2 text-xs text-midnight-navy/50 whitespace-nowrap">
                  Coming Soon
                </span>
              </button>
