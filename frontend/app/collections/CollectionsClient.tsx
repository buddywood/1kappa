...
interface CollectionsClientProps {
  ...
  sellerIdParam: number | null;
}

type SortOption = 'name' | 'product-count' | 'newest';
...
export default function CollectionsClient({ 
  ...
  const [expandedSellers, setExpandedSellers] = useState<Set<number>>(
    sellerIdParam ? new Set([sellerIdParam]) : new Set()
  );
  ...
  const toggleSellerExpansion = (sellerId: number) => {
  ...
  const filteredAndSortedSellers = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter(seller => {
        const displayName = (seller.business_name || seller.name).toLowerCase();
        const sellerName = seller.name.toLowerCase();
        return displayName.includes(query) || sellerName.includes(query);
      });
    }

    // Filter by chapter
    if (selectedChapter) {
      filtered = filtered.filter(seller => seller.sponsoring_chapter_id === selectedChapter);
    }

    // Filter by category (filter sellers that have products in the selected category)
    if (selectedCategory) {
      filtered = filtered.filter(seller => 
        seller.products.some(product => product.category_id === selectedCategory)
      );
    }
    ...
    filtered.sort((a, b) => {
      switch (sortBy) {
        ...
        case 'newest':
          // Sort by newest product (if available)
          const aNewest = a.products[0]?.id || 0;
          const bNewest = b.products[0]?.id || 0;
    ...
    return filtered;
  }, [sellers, searchQuery, selectedChapter, selectedCategory, sortBy]);

  // Filter products within each seller's collection
  ...
  const totalProducts = sellers.reduce((sum, seller) => sum + seller.product_count, 0);

  // Handle case where sellerIdParam is provided but seller not found
  if (sellerIdParam && filteredAndSortedSellers.length === 0 && sellers.length > 0) {
    return (
    ...
    <>
      ...
      <section className="max-w-7xl mx-auto px-4 py-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          <div className="bg-white rounded-xl p-6 shadow-md border border-frost-gray">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-full bg-crimson/10 flex items-center justify-center">
                <svg className="w-6 h-6 text-crimson" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-3-3h-4a3 3 0 000 3zM9 10a3 3 0 100-6 3 3 0 000 6zM19 10a3 3 0 100-6 3 3 0 000 6z" />
                </svg>
              </div>
              <div>
                <p className="text-2xl font-bold text-midnight-navy">{totalSellers}</p>
                <p className="text-sm text-midnight-navy/60">Active Sellers</p>
              </div>
            </div>
          </div>
          <div className="bg-white rounded-xl p-6 shadow-md border border-frost-gray">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-full bg-aurora-gold/10 flex items-center justify-center">
                <svg className="w-6 h-6 text-aurora-gold" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" />
                </svg>
              </div>
              <div>
                <p className="text-2xl font-bold text-midnight-navy">{totalProducts}</p>
                <p className="text-sm text-midnight-navy/60">Total Products</p>
              </div>
            </div>
          </div>
          <div className="bg-white rounded-xl p-6 shadow-md border border-frost-gray">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-full bg-midnight-navy/10 flex items-center justify-center">
              ...
              <div>
                <p className="text-2xl font-bold text-midnight-navy">
                  {chapters.filter(c => sellers.some(s => s.sponsoring_chapter_id === c.id)).length}
                </p>
                <p className="text-sm text-midnight-navy/60">Chapters Represented</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="max-w-7xl mx-auto px-4 pb-8">
        <div className="bg-white rounded-xl shadow-md border border-frost-gray p-6">
          <div className="space-y-4">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-3 border border-frost-gray rounded-lg focus:ring-2 focus:ring-crimson focus:border-transparent text-midnight-navy bg-cream"
              />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <SearchableSelect
                ...
                value={selectedChapter ? selectedChapter.toString() : ''}
              ...
              <SearchableSelect
                options={[
                  ...
                  { id: 'product-count', label: 'Most Products', value: 'product-count' },
            ...
            {/* Results Count */}
            <div className="text-sm text-midnight-navy/60 pt-2 border-t border-frost-gray">
              Showing {filteredAndSortedSellers.length} {filteredAndSortedSellers.length === 1 ? 'collection' : 'collections'}
        ...
        {filteredAndSortedSellers.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-xl border border-frost-gray">
            <div className="max-w-md mx-auto">
              ...
              <p className="text-midnight-navy/70 mb-6">
                {searchQuery || selectedChapter || selectedCategory
          ...
          <div className="space-y-8">
            {filteredAndSortedSellers.map((seller) => {
              ...
              const filteredProducts = getFilteredProducts(seller.products);
              const isExpanded = expandedSellers.has(seller.id);
              const showProducts = isExpanded || sellerIdParam === seller.id;
              
              return (
                <div 
                  key={seller.id} 
                  id={`seller-${seller.id}`} 
                  className="bg-white rounded-2xl shadow-lg overflow-hidden border border-frost-gray scroll-mt-24 transition-all hover:shadow-xl"
                >
                  {/* Seller Header */}
                    ...
                    <div className="flex flex-col md:flex-row items-start md:items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-2 mb-2">
                          ...
                          <span className="bg-crimson/10 text-crimson text-xs font-semibold px-2 py-1 rounded-full">
                      ...
                      <Link
                        ...
                        className="flex items-center gap-2 text-crimson hover:text-crimson/80 transition font-semibold text-sm"
                      >
                        View Store
                        <svg 
                            className="w-5 h-5"
