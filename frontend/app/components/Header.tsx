...
function HeaderContent() {
  ...
  const [memberVerificationStatus, setMemberVerificationStatus] = useState<
    string | null
  >(null);
  const {
    items,
  ...
  const showAuthenticatedMenu =
    sessionStatus === "authenticated" && nextAuthSession?.user;
  ...
  const userRole =
    (session?.user as any)?.role ?? (nextAuthSession?.user as any)?.role;
  const memberId =
    (session?.user as any)?.memberId ??
    (nextAuthSession?.user as any)?.memberId;
  const sellerId =
    (session?.user as any)?.sellerId ??
    (nextAuthSession?.user as any)?.sellerId;
  const promoterId =
    (session?.user as any)?.promoterId ??
    (nextAuthSession?.user as any)?.promoterId;
  const stewardId =
    (session?.user as any)?.stewardId ??
    (nextAuthSession?.user as any)?.stewardId;
  // Get role flags from session - check both session sources
  const is_fraternity_member =
    (session?.user as any)?.is_fraternity_member ??
    (nextAuthSession?.user as any)?.is_fraternity_member ??
    false;
  const is_seller =
    (session?.user as any)?.is_seller ??
    (nextAuthSession?.user as any)?.is_seller ??
    false;
  const is_promoter =
    (session?.user as any)?.is_promoter ??
    (nextAuthSession?.user as any)?.is_promoter ??
    false;
  const is_steward =
    (session?.user as any)?.is_steward ??
    (nextAuthSession?.user as any)?.is_steward ??
    false;
  ...
  const finalShowBecomeMember =
    (!showAuthenticatedMenu || (is_seller && !is_fraternity_member)) &&
    !is_promoter &&
  ...
  const isLinkActive = (link: (typeof navLinks)[0]) => {
    if (link.matchPath) {
      ...
      if (link.href === "/events") {
        const isPromoterEventsPage = pathname?.startsWith(
        ...
        <div className="flex items-center justify-between h-14">
          ...
          >
            ...
            <Image
              ...
              priority
            />
            <span className="font-display font-bold text-crimson text-xl whitespace-nowrap">
              1KAPPA
            </span>
          </Link>
          ...
          <div className="hidden lg:flex items-center gap-4 flex-shrink-0 ml-4">
            ...
            <form onSubmit={handleSearch} className="relative">
              <div
                className={`flex items-center border rounded-lg transition-all bg-white dark:bg-black ${
                  searchFocused
                    ? "border-crimson shadow-sm dark:border-crimson"
                    ...
                    <>
                      <div className="max-h-96 overflow-y-auto">
                        {items.map((item) => (
                          <div
                            key={item.product.id}
                            className="p-4 border-b border-gray-200 dark:border-gray-900 flex gap-3"
                          >
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-semibold text-gray-900 dark:text-gray-100 truncate">
                            {userRole === "GUEST"
                              ? firstName ||
                                (nextAuthSession?.user as any)?.email?.split(
                                  "@"
                                )[0] ||
                                "Account"
                              : firstName
